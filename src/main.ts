...
async function main() {
    ...
    const platform = new ChatrixPlatform({container: root, assetPaths, config: {}, options: { development: import.meta.env.DEV }});
...
function allowsChild(parent, child) {
    ...
    switch (parent?.type) {
        case undefined:
            return type === "start" || type === "login" || type === "settings" || type === "account-setup" || type === "timeline" || type === "minimize";
        default:
            return false;
    }
}
...
(window as any).sendViewChangeToParent = function (view: "timeline" | "login" | "settings" | "account-setup") {
