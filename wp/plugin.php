...
function a8c_orbit_chatterbox_settings() {
    ...
    add_settings_field(
        ...
        'Default Matrix room',
		function() {
			$settings = get_option( 'a8c_orbit_chatterbox_settings' );
			...
			if ( $modified ) {
				update_option( 'a8c_orbit_chatterbox_settings', $settings );
			}

			?>
			<input type="text" name="a8c_orbit_chatterbox_settings[default_matrix_room]" value="<?php echo isset( $settings['default_matrix_room'] ) ? esc_attr( $settings['default_matrix_room'] ) : ''; ?>" class="regular-text">
			...
			<?php foreach ( $settings['overrides'] as $key => $override ) { ?>
				<div class="a8c-orbit-chatterbox-overrides">
					<p class="description">Embed this room
						<input type="text" name="a8c_orbit_chatterbox_settings[overrides][<?php echo $key ?>][room]" value="<?php echo isset( $override['room'] ) ? esc_attr( $override['room'] ) : ''; ?>" class="regular-text">
						on these pages:

						<select multiple name="a8c_orbit_chatterbox_settings[overrides][<?php echo $key ?>][pages][]">
			...
			<div class="a8c-orbit-chatterbox-overrides">
				<p class="description">Embed this room
					<input type="text" name="a8c_orbit_chatterbox_settings[overrides][<?php echo $key ?>][room]" value="" class="regular-text">
					on these pages:
